...
class Sentence:
    def __init__(self, verb: SentenceVerb, target: SentenceNoun, attribute: SentenceAttribute):
        self.verb = verb
        self.target = target
    ...
    def is_transformation(self) -> bool:
        return (self.verb.object_type == GameObjectType.T_IS and
                self.attribute.object_type.get_category() == GOCategory.NOUN)
...
def parse_sentence(words: List[GameObject]) -> Optional[Sentence]:
    ...
    def get_phrase() -> Optional[Sentence]:
        ...
        return Sentence(verb, noun, target)
